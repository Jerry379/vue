...
const resolve = (p) => {
  const base = p.split("/")[0]; //取数组的第一个元素设置为base
  if (aliases[base]) {
...
const builds = {
  ...
  "web-runtime-cjs-dev": {
    ...
    dest: resolve("dist/vue.runtime.common.dev.js"), // 构建后的js地址
    format: "cjs", // 构建格式：cjs 表示构建出来的文件遵循 CommonJS 规范，es 表示构建出来的文件遵循 ES Module 规范。 umd 表示构建出来的文件遵循 UMD 规范
    env: "development",
    banner,
  },
